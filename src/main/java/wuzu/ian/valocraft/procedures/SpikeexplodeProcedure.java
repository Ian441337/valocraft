...
public class SpikeexplodeProcedure {
	...
	private static boolean begin(VertexFormat.Mode mode, boolean texture, boolean update) {
		if (bufferBuilder == null || !bufferBuilder.building()) {
			...
			if (vertexBuffer == null) {
				SpikeexplodeProcedure.texture = texture;
	...
	private static void system(boolean worldCoordinate) {
		SpikeexplodeProcedure.worldCoordinate = worldCoordinate;
	}

	private static boolean target(int targetStage) {
		if (targetStage == currentStage) {
			SpikeexplodeProcedure.targetStage = targetStage;
			return true;
		}
		return false;
	}
	...
	private static void execute(@Nullable Event event, LevelAccessor world) {
		...
		if (target(2)) {
			renderShape((shape()), ValocraftModVariables.MapVariables.get(world).spike_x, ValocraftModVariables.MapVariables.get(world).spike_y, ValocraftModVariables.MapVariables.get(world).spike_z, 0, 0, 0, 35, 35, 35,
					255 << 24 | 0 << 16 | 0 << 8 | 0);
			release();
		}
	}
